...
class MuZero:
    ...
    def __init__(self, game_name, config=None, split_resources_in=1):
        ...
        ray.init(num_gpus=total_gpus, ignore_reinit_error=True, object_store_memory=3e+10)
    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            while info["training_step"] < self.config.training_steps:
                ...
                writer.add_scalar(
                    "2.Workers/6.Training_steps_per_played_games_ratio",
                    info["training_step"] / max(1, info["num_played_games"]),
                    counter,
                )
                ...
                writer.add_scalar("3.Loss/Entropy_loss", info["entropy_loss"], counter)
                byte_to_gb = 1024 ** 3
                writer.add_scalar("4.System/Available_RAM", psutil.virtual_memory().available / byte_to_gb, counter)
                writer.add_scalar("4.System/Percent_usage_RAM", psutil.virtual_memory().percent / byte_to_gb, counter)
                nvidia_smi.nvmlInit()
                handle = nvidia_smi.nvmlDeviceGetHandleByIndex(0)
                gpu_info = nvidia_smi.nvmlDeviceGetMemoryInfo(handle)
                writer.add_scalar("4.System/Available_GPU_RAM", gpu_info.free / byte_to_gb, counter)
                writer.add_scalar("4.System/Percent_usage_GPU_RAM", round(gpu_info.used / gpu_info.total * 100,2), counter)
