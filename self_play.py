...
class GameHistory:
    ...
    #     # print(index, num_stacked_observations)
    #     index = index % len(self.observation_history)
    #     # print(index, len(self.observation_history))
    #     stacked_observations = self.observation_history[index].copy()
    #     print(stacked_observations[0].shape, self.observation_history[index].shape, self.action_history[0].shape)
    ...
    def get_stacked_observations(
            self, index, num_stacked_observations, action_space_size=2
        ):
        """
        Generate a new observation with the observation at the index position
        and num_stacked_observations past observations and actions stacked.
        ...
        # Convert to positive index
        index = index % len(self.observation_history)
        # print(self.action_history[0], action_space_size, [(ah, ah / action_space_size) for ah in self.action_history])
        stacked_observations = self.observation_history[index].copy()
        for past_observation_index in reversed(
            range(index - num_stacked_observations, index)
        ):
            if 0 <= past_observation_index:
                # print(stacked_observations[0].shape, self.action_history[past_observation_index + 1].shape, self.observation_history[past_observation_index].shape)
                previous_observation = numpy.concatenate(
                    (
                        self.observation_history[past_observation_index],
                        [
                            ...
                            # / action_space_size
                            numpy.ones_like(stacked_observations[0])
                            * self.action_history[past_observation_index + 1][0] # NEM EREDETI
                            / action_space_size
                        ],
                    )
                )
